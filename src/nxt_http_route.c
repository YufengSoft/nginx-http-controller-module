...
static nxt_http_route_match_t *nxt_http_route_match_create(nxt_mp_t *mp,
    nxt_conf_value_t *cv);
static nxt_http_route_table_t *nxt_http_route_table_create(nxt_mp_t *mp,
    nxt_conf_value_t *table_cv, nxt_http_route_object_t object,
    nxt_bool_t case_sensitive);
static nxt_http_route_ruleset_t *nxt_http_route_ruleset_create(nxt_mp_t *mp,
    nxt_conf_value_t *ruleset_cv, nxt_http_route_object_t object,
    nxt_bool_t case_sensitive);
static nxt_http_route_rule_t *nxt_http_route_rule_name_create(nxt_mp_t *mp,
    nxt_conf_value_t *rule_cv, nxt_str_t *name, nxt_bool_t case_sensitive);
static nxt_http_route_rule_t *nxt_http_route_rule_create(nxt_mp_t *mp,
...
static nxt_int_t nxt_http_route_scheme(nxt_http_request_t *r,
    nxt_http_route_rule_t *rule);
static nxt_int_t nxt_http_route_test_rule(nxt_http_route_rule_t *rule,
    u_char *start, size_t length);
static nxt_int_t nxt_http_route_pattern(nxt_http_route_pattern_t *pattern,
    u_char *start, size_t length);
...
nxt_http_routes_t *
nxt_http_routes_create(nxt_mp_t *mp, nxt_conf_value_t *routes_conf)
{
    size_t                  size;
    uint32_t                i, n;
    ...
    size = sizeof(nxt_http_routes_t) + n * sizeof(nxt_http_route_match_t *);
    ...
    for (i = 0; i < n; i++) {
        ...
        match = nxt_http_route_match_create(mp, value);
        if (match == NULL) {
            return NULL;
        }
...
nxt_http_route_match_create(nxt_mp_t *mp, nxt_conf_value_t *cv)
{
    size_t                       size;
    uint32_t                     n;
    nxt_int_t                    ret;
    ...
    ret = nxt_conf_map_object(mp, match_conf, nxt_http_route_match_conf,
    ...
    if (mtcf.scheme != NULL) {
        rule = nxt_http_route_rule_create(mp, mtcf.scheme, 1,
    ...
    if (mtcf.host != NULL) {
        rule = nxt_http_route_rule_create(mp, mtcf.host, 1,
                                          NXT_HTTP_ROUTE_PATTERN_LOWCASE);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.uri != NULL) {
        rule = nxt_http_route_rule_create(mp, mtcf.uri, 1,
                                          NXT_HTTP_ROUTE_PATTERN_NOCASE);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.method != NULL) {
        rule = nxt_http_route_rule_create(mp, mtcf.method, 1,
                                          NXT_HTTP_ROUTE_PATTERN_UPCASE);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.headers != NULL) {
        table = nxt_http_route_table_create(mp, mtcf.headers,
    ...
    if (mtcf.arguments != NULL) {
        table = nxt_http_route_table_create(mp, mtcf.arguments,
...
action_create:
    ...
    ret = nxt_http_route_action_create(mp, cv, match);
...
nxt_http_route_table_create(nxt_mp_t *mp, nxt_conf_value_t *table_cv,
    nxt_http_route_object_t object, nxt_bool_t case_sensitive)
{
    size_t                    size;
    uint32_t                  i, n;
    nxt_bool_t                array;
    ...
    if (!array) {
        ruleset = nxt_http_route_ruleset_create(mp, table_cv, object, case_sensitive);
    ...
    for (i = 0; i < n; i++) {
        ...
        ruleset = nxt_http_route_ruleset_create(mp, ruleset_cv, object, case_sensitive);
        if (nxt_slow_path(ruleset == NULL)) {
            return NULL;
        }
...
nxt_http_route_ruleset_create(nxt_mp_t *mp, nxt_conf_value_t *ruleset_cv,
    nxt_http_route_object_t object, nxt_bool_t case_sensitive)
{
    size_t                    size;
    uint32_t                  i, n, next;
    ...
    for (i = 0; i < n; i++) {
        ...
        rule = nxt_http_route_rule_name_create(mp, rule_cv, &name, case_sensitive);
...
nxt_http_route_rule_name_create(nxt_mp_t *mp, nxt_conf_value_t *rule_cv,
    nxt_str_t *name, nxt_bool_t case_sensitive)
{
    ...
    rule = nxt_http_route_rule_create(mp, rule_cv, case_sensitive,
                                      NXT_HTTP_ROUTE_PATTERN_NOCASE);
    if (nxt_slow_path(rule == NULL)) {
        return NULL;
    }
...
static nxt_http_route_rule_t *
nxt_http_route_rule_create(nxt_mp_t *mp, nxt_conf_value_t *cv,
    nxt_bool_t case_sensitive, nxt_http_route_pattern_case_t pattern_case)
{
    ...
    if (string) {
        ...
        ret = nxt_http_route_pattern_create(mp, cv, &pattern[0], pattern_case);
        if (nxt_slow_path(ret != NXT_OK)) {
            return NULL;
        }
    ...
    for (i = 0; i < n; i++) {
        ...
        ret = nxt_http_route_pattern_create(mp, value, &pattern[i], pattern_case);
        if (nxt_slow_path(ret != NXT_OK)) {
            return NULL;
        }
    }
...
static nxt_int_t
nxt_http_route_pattern_create(nxt_mp_t *mp, nxt_conf_value_t *cv,
    nxt_http_route_pattern_t *pattern, nxt_http_route_pattern_case_t pattern_case)
{
    u_char                         *start;
    nxt_str_t                      test;
    nxt_uint_t                     n, length;
...
static nxt_conf_map_t  nxt_http_route_action_conf[] = {

    {
        ...
        NXT_CONF_MAP_PTR,
        offsetof(nxt_http_route_action_conf_t, variables)
    },

    {
        nxt_string("blacklist"),
        NXT_CONF_MAP_PTR,
        offsetof(nxt_http_route_action_conf_t, blacklist),
    },

    {
        nxt_string("whitelist"),
        NXT_CONF_MAP_PTR,
        offsetof(nxt_http_route_action_conf_t, whitelist),
    },

    {
        nxt_string("add_headers"),
        NXT_CONF_MAP_PTR,
        offsetof(nxt_http_route_action_conf_t, add_headers)
    },
};


static nxt_int_t
nxt_http_route_action_create(nxt_mp_t *mp, nxt_conf_value_t *cv,
    nxt_http_route_match_t *match)
{
    size_t                         size;
    uint32_t                       i, n, next;
    ...
    ret = nxt_conf_map_object(mp, action_conf, nxt_http_route_action_conf,
    ...
    if (variables_conf != NULL) {
        ...
        size = sizeof(nxt_http_action_variables_t)
               + n * sizeof(nxt_http_name_value_t *);
        ...
        for (i = 0; i < n; i++) {
            nv = nxt_mp_alloc(mp, sizeof(nxt_http_name_value_t));
    ...
    if (headers_conf != NULL) {
        ...
        size = sizeof(nxt_http_action_headers_t)
               + n * sizeof(nxt_http_name_value_t *);
...
nxt_http_action_t *
nxt_http_route_action(nxt_http_request_t *r, nxt_http_routes_t *routes)
{
    nxt_http_action_t       *action;
...
nxt_http_route_rule(nxt_http_request_t *r, nxt_http_route_rule_t *rule)
{
    ...
    return nxt_http_route_test_rule(rule, start, length);
}
...
nxt_http_route_header(nxt_http_request_t *r, nxt_http_route_rule_t *rule)
{
    ...
    nxt_list_each(f, r->fields) {
        ...
        ret = nxt_http_route_test_rule(rule, f->value, f->value_length);
...
nxt_http_route_arguments(nxt_http_request_t *r, nxt_http_route_rule_t *rule)
{
    ...
    while (nv < end) {
        ...
        {
            ret = nxt_http_route_test_rule(rule, nv->value, nv->value_length);
            if (ret == 0) {
                break;
            }
        }
...
static nxt_int_t
nxt_http_route_test_rule(nxt_http_route_rule_t *rule, u_char *start,
    size_t length)
{
    nxt_int_t                 ret;
    ...
    while (pattern < end) {
        ret = nxt_http_route_pattern(pattern, start, length);
...
static nxt_int_t
nxt_http_route_pattern(nxt_http_route_pattern_t *pattern, u_char *start,
    size_t length)
{
    u_char     *p, *end, *test;
    size_t     test_length;
