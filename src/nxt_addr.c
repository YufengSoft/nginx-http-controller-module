...
nxt_int_t
nxt_addr_pattern_parse(nxt_mp_t *mp, nxt_addr_pattern_t *pattern,
    nxt_conf_value_t *cv)
{
    u_char                 *delim;
    nxt_int_t              ret, cidr_prefix;
    nxt_str_t              addr;
    nxt_addr_range_t       *inet;
    ...
    if (nxt_str_looks_like_ipv6(&addr)) {
        ...
        if (delim != NULL) {
            ...
            pattern->match_type = NXT_ADDR_RANGE;
        ...
        if (delim != NULL) {
            ...
            if (nxt_slow_path(cidr_prefix == 0)) {
                pattern->match_type = NXT_ADDR_ANY;

                return NXT_OK;
            }

            if (nxt_slow_path(cidr_prefix == 128)) {
                pattern->match_type = NXT_ADDR_EXACT;

                return NXT_OK;
            }

            pattern->match_type = NXT_ADDR_CIDR;
...
nxt_int_t
nxt_addr_pattern_match(nxt_addr_pattern_t *p, nxt_sockaddr_t *sa)
{
#if (NXT_INET6)
    uint32_t              i;
#endif
